...
import { ChevronRight, Mail, Sparkles, RefreshCcw, ShieldCheck, ArrowLeft } from 'lucide-react';
      ...
      <div className="hidden lg:flex w-1/2 relative overflow-hidden flex-col justify-between p-12 border-r border-white/5 bg-bg-dark-1">
        ...
        <div className="relative z-10 max-w-lg mt-12">
          ...
          <div className="space-y-6">
            {[
              { title: "One-Time Password", desc: "A 6-digit code has been sent to your email.", icon: ShieldCheck },
              { title: "Account Protection", desc: "Verifying your email ensures your account is secure.", icon: Mail },
              { title: "Almost Ready", desc: "Complete this step to start exploring Trackr.", icon: Sparkles }
            ...
            <div className="flex justify-between gap-3">
              {otp.map((digit, index) => (
                <div key={index} className={`flex-1 transition-all duration-200 ${activeInput === index ? 'scale-105' : ''}`}>
                  <div className={`relative group rounded-xl transition-all duration-300 ${activeInput === index ? 'p-px bg-linear-to-r from-indigo-500 via-purple-500 to-indigo-500 bg-size-[200%_100%] animate-[shimmer_2s_linear_infinite]' : 'p-px bg-white/10 hover:bg-white/20'}`}>
              ...
              <button
                ...
                className={`relative group flex items-center gap-3 px-6 py-2.5 rounded-full border transition-all duration-300 ${
                  timer > 0 || isResending 
                    ...
                    : 'bg-white/5 border-white/10 text-indigo-400 hover:bg-white/10 hover:border-indigo-500/30'
                }`}
              >
                {timer > 0 ? (
                  <div className="relative flex items-center justify-center w-5 h-5">
                    ...
                    <span className="absolute text-[8px] font-bold text-indigo-400">{timer}</span>
                  </div>
                ) : (
                ...
                <span className="text-sm font-semibold">
                  {isResending ? 'Sending...' : timer > 0 ? 'Wait to resend' : 'Resend code'}
                </span>
              </button>
            </div>
          </div>
